...
const config = {
 models:{title:'能源模型',value:'从组件到系统',desc:'按能源方向组织模型<br>配套参数与验证资料',eyebrow:'模型资源',english:'EnerV Model',heading:'按能源方向，查看模型分类',intro:'按方向查模型，按需求确认资料。',action:'提交模型需求'},
 twin:{title:'数智孪生',value:'从规划到优化',desc:'连接模型数据与分析<br>呈现软件应用能力',eyebrow:'数智平台',english:'EnerV Systems',heading:'三类业务，串联六个产品',intro:'贯通模型与数据，按场景组合应用。',action:'提交软件项目需求'},
 teaching:{title:'教学软件',value:'从原理到实践',desc:'围绕建模运行与分析<br>组织能源教学实践',eyebrow:'虚拟实验',english:'Lab',heading:'三阶段教学，五步实践',intro:'理解原理、动手实践、记录成果。',action:'提交教学需求'},
 services:{title:'技术服务',value:'从建模到验证',desc:'围绕模型与仿真验证<br>按项目需求组织实施',eyebrow:'工程服务',english:'Engineering Services',heading:'三类业务，八项服务方向',intro:'明确任务与条件，约定成果与验收。',action:'提交服务需求'}
};
const c=config[page];
let viewGroup='all';
...
function render(){
 document.getElementById('app').innerHTML=`<header class="hero"><div class="brand"><img src="/wechat/tech-exhibition-v4/logo.png" alt="星旭新能源标志"><span>星旭新能源</span></div><div class="hero-layout"><div><h1><span>${c.title}</span><span>${c.value}</span></h1><i class="hero-rule" aria-hidden="true"></i><p>${c.desc}</p></div>${art()}</div></header><nav class="tabs" style="--tabs:${Object.keys(groups()).length+1}" aria-label="${c.title}分类">${Object.entries({all:{label:'全部'},...groups()}).map(([key,item],index)=>`<button data-group="${key}" aria-pressed="${key===viewGroup}"><span class="nav-number" aria-hidden="true">${String(index+1).padStart(2,'0')}</span><i class="nav-node" aria-hidden="true"></i><span class="nav-label">${item.label}</span></button>`).join('')}</nav><div class="content"><section class="overview"><div class="eyebrow"><span>${c.eyebrow}</span><small lang="en">${c.english}</small></div><h2>${c.heading}</h2></section>${catalogue()}${detail()}${extra()}${scenarioSection()}${footer()}</div>`;
}
